				...
				<table id="educationTable">
					...
					<c:if test="${educationDtoList == null}">
						<tr class="educationTr">
							...
							<td class="resumeValue"><input type="hidden" name="resumeEduNoValue" value="0"></td>
						</tr>
					</c:if>
...
<%-- 					${careerDtoList} --%>
					<c:if test="${careerDtoList == null}">
						<tr class="careerTr">
							...
							<td class="resumeValue"><input type="hidden" name="resumeCareerNoValue" value="0"></td>
						</tr>                      
					</c:if>
					<c:forEach var="careerDto" items="${careerDtoList}">
						<tr class="careerTr">
							...
							<td class="resumeValue"><input type="hidden" name="resumeCareerNoValue" value="${careerDto.carNo}"></td>
	...
	<%-- 					${resumeDto.resumeLicenseDate} --%>
				...
				<input type="hidden" id="dtoList" name="requestDto" value="">
<!-- 				<input type="text" id="careerDtoV2" name="careerDto" value=""> -->
<!-- 				<input type="text" id="educationDtoV2" name="educationDto" value=""> -->
				<c:if test="${resumeDto.resumeNo != null}">
					<input type="hidden" id="resumeNo" name="resumeNo" value="${resumeDto.resumeNo}">
				</c:if>
				<c:if test="${resumeDto.resumeNo == null}">
					<input type="hidden" id="resumeNo" name="resumeNo" value="0">
				</c:if>
				<input type="hidden" id="perNo" name="perNo" value="${sessionScope.personalMemberDto.perNo}">
			</form>
		</c:if>
		<c:if test="${resumeDto == null}">		
			<form id ="resumeUpdateForm" action="./resumeAddCtr.do" method="post">
			<table>
					<thead>
						<tr>
							<th>개인 정보</th>
							...
							<th>
							<input type="button" id="cancelButton" onclick="cancelFnc();" value="취소">
							<input type="button" id="submitButton" onclick="submitFnc();" value="작성 완료">
							</th>
